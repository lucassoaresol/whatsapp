...
export const retriveChatWpp = async (clientId: string, chatId: string) => {
  const { data } = await apiUsingNow.get<IChatWpp>(
    `${clientId}/chats/${chatId}/wpp`,
  );
...
export const retriveMessageWpp = async (clientId: string, msgId: string) => {
  const { data } = await apiUsingNow.get<IMessageWpp>(
    `${clientId}/messages/${msgId}`,
  );

  return data;
};

export const retriveMessageContactWpp = async (
  clientId: string,
  msgId: string,
) => {
...
export const retriveMessageMediaWpp = async (
  clientId: string,
  msgId: string,
) => {
...
export const sendMessage = async (
  ...
  await apiUsingNow.post(`${clientId}/messages`, { number, message, mentions });
};
